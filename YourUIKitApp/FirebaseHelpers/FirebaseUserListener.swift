...
class FirebaseUserListener {
    ...
    func registerUserWith(
        email: String, password: String, completion: @escaping (_ error: Error?) -> Void
    ) {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (authDataResult, error) in
            ...
            if error == nil {
                ...
                if authDataResult?.user != nil {
                    ...
                    let user = User(
                        id: authDataResult!.user.uid, username: email, email: email, pushId: "",
                        avatarLink: "", status: "Hey there I'm using Messager")
    ...
    func saveUserToFireStore(_ user: User) {
        ...
        //        do {
        //            try FirebaseReference(.User).document(user.id).setData(user)
        //        } catch {
    ...
    func downloadUsersFromFirebase(
        withIds: [String], completion: @escaping (_ allUsers: [User]) -> Void
    ) {
    ...
    func updateUserInFirebase(_ user: User) {
        ...
        //        do {
        //            let _ = try FirebaseReference(.User).document(user.id).setData(user)
        //        } catch {
